...
from conexao_bd import get_conexao_postgres

def criar_tabela_cidade():
    try:
        conexao = get_conexao_postgres('rodoviaria', 'postgres', '123')
        ...
        cursor.executemany("""
        INSERT INTO cidade (nome, estado, ccodigo)
        VALUES (%s, %s, %s);
        """, cidades)

        cursor.executemany("""
        INSERT INTO empresa (cnpj, nome, cidade)
        VALUES (%s, %s, %s);
        """, empresas)

        cursor.executemany("""
        INSERT INTO cliente (pnome, unome, rg, cpf, data_nasc, telefone)
        VALUES (%s, %s, %s, %s, %s, %s);
        """, clientes)

        cursor.executemany("""
        INSERT INTO funcionario (fcodigo, senha, pnome, unome, rg, cpf, empresa, cidade)
        VALUES (%s, %s, %s, %s, %s, %s, %s, %s);
        """, funcionarios)

        cursor.executemany("""
        INSERT INTO onibus (nonibus, placa, empresa, modelo, ano, classe)
        VALUES (%s, %s, %s, %s, %s, %s);
        """, onibus)

        cursor.executemany("""
        INSERT INTO viagem (cviagem, nonibus, data, horario, origem, destino, preco)
        VALUES (%s, %s, %s, %s, %s, %s, %s);
        """, viagens)

        cursor.executemany("""
        INSERT INTO motorista (pnome, unome, rg, cpf, nmotorista, cviagem)
        VALUES (%s, %s, %s, %s, %s, %s);
        """, motoristas)

        cursor.executemany("""
        INSERT INTO bilhete (nbilhete, rg_cliente, data_venda, cviagem, vendedor)
        VALUES (%s, %s, %s, %s, %s);
        """, bilhetes)
        ...
        print("Tabelas rodoviaria criada e populada com sucesso!")
    except (Exception, psycopg2.DatabaseError) as erro:
        print(f"Erro ao criar a tabelas rodoviaria: {erro}")
